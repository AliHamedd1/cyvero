...
export async function GET(_: Request, context: Context) {
  try {
    ...
    return NextResponse.json({ error: "تعذر تحميل المحادثة حاليًا." }, { status: 500 });
  }
}
...
export async function PATCH(request: Request, context: Context) {
  try {
      ...
      updatedConversation = {
        ...
        messages: [
          ...updatedConversation.messages,
          {
            ...
            sender: "system",
            senderName: "Cyvero",
            body: `أضاف المختص عرضًا سعريًا جديدًا بقيمة ${price} ريال ومدة تنفيذ ${durationDays} يومًا.`,
            sentAt: now,
          },
        ],
      };
    ...
    return NextResponse.json({ error: "تعذر تحديث حالة الطلب حاليًا." }, { status: 500 });
  }
}
