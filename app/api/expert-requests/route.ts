...
export async function POST(request: Request) {
  try {
    ...
    const requests = await readRuntimeCollection<ExpertRequestSubmission[]>(runtimeFiles.expertRequests, []);
    ...
    return NextResponse.json({ error: "تعذر معالجة طلب المختص حاليًا." }, { status: 500 });
  }
}
